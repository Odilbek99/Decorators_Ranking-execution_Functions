...
class task4_decorator:
    ...
    def __call__(self,*args):
        ...
        try:
            ...
            with open('output_programs.txt', 'a') as file:
                with redirect_stdout(file) as f:  # puts functions outputs into text.txt file
                    ...
                    print(s)
                    task4_decorator.d[(self.decorator_argument.__name__, id(self.decorator_argument))] = execution_time
        except Exception as err:
            with open('task4_errors.txt', 'a') as e:
                with redirect_stdout(e) as f:
                    print(datetime.now(), err)


    def table(self):
        all_functions = dict(sorted(task4_decorator.d.items(), key=lambda item: item[1]))
